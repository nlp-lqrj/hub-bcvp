...
def build_sample(vocab, window_size, corpus):
    ...
    # print(window, target)
    x = [vocab.get(word, vocab.get("<UNK>", 0)) for word in window]   #将字转换成序号
    y = [vocab.get(word, vocab.get("<UNK>", 0)) for word in target]
    return x, y
...
def generate_sentence(openings, model, vocab, window_size):
    ...
    with torch.no_grad():
        ...
        while pred_char != "\n" and len(openings) <= 30:
            openings += pred_char
            x = [vocab.get(char, vocab.get("<UNK>", 0)) for char in openings[-window_size:]]
