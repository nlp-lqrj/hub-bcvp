...
# 重听功能：用户说这些内容时重复上一条回复
REPLAY_KEYWORDS = ["重听", "再说一遍", "重复一下", "没听清", "再说一次", "重复"]
...
class DialogueSystem:
    ...
    def run(self, query, memory):
        ...
        '''
        memory["query"] = query
        # 重听：直接返回上一条回复，不改变对话状态
        if self.is_replay_request(query):
            memory["response"] = memory.get(
                "last_response", "抱歉，没有上一条回复可以重播。"
            )
            return memory
